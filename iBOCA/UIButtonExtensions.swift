    ...
    func addTextSpacing(_ spacing: CGFloat, for state: UIControl.State = .normal, useDefaultFontColor: Bool) {
        ...
        if useDefaultFontColor {
            if let textColor = self.titleColor(for: state) {
                attributedString.addAttribute(NSAttributedString.Key.init(string: "foregroundColor") as String, value: textColor, range: NSRange(location: 0, length: attributedString.length))
            }
            if let font = self.titleLabel?.font {
                attributedString.addAttribute(NSAttributedString.Key.init(string: "font") as String, value: font, range: NSRange(location: 0, length: attributedString.length))
            }
        }
        self.setAttributedTitle(attributedString, for: state)
    }
